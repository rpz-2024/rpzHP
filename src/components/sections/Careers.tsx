...
const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div
    className={`rounded-3xl border border-neutral-200 bg-white p-5 md:p-6 shadow-sm ${className ?? ""}`}
  >
    {children}
  </div>
);

const PillAnchorNav: React.FC<{ items: { href: string; label: string }[] }> = ({
  items,
}) => (
  <nav className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
    {items.map((it) => (
      <a
        key={it.href}
        href={it.href}
        className="px-3 py-1.5 rounded-full border border-neutral-300 text-sm hover:bg-neutral-50 transition"
      >
        {it.label}
      </a>
    ))}
...
const CareerTimeline: React.FC<{ steps: Occupation["steps"] }> = ({
  steps,
}) => (
  <ol className="relative border-l border-neutral-200 pl-4 md:pl-6 space-y-6">
    {steps.map((s, i) => (
      <li key={i} className="ml-2">
        ...
        <p className="mt-1 text-sm md:text-base text-neutral-700 whitespace-pre-line">
          {s.body}
        </p>
      </li>
    ))}
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
        ...
        <div className="flex-1">
          <div className="flex items-baseline gap-3">
            ...
            {data.enTitle && (
              <span className="text-xs md:text-sm tracking-widest text-neutral-500 uppercase">
                {data.enTitle}
              </span>
            )}
          </div>
          <p className="mt-3 text-sm md:text-base text-neutral-800 leading-relaxed">
            {data.summary}
          </p>
    ...
    <div className="space-y-8 md:space-y-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {c.jobSpecs.map((it, idx) => (
            <Card key={idx}>
              <div className="text-[11px] tracking-widest text-neutral-500 font-semibold uppercase">
                {it.label}
              </div>
              <div className="mt-1 text-[15px] md:text-base font-medium text-neutral-900">
                {it.value}
              </div>
            </Card>
          ))}
      ...
      <div>
        ...
        <p className="text-neutral-600 text-sm md:text-base mb-4">
          それぞれの役割とキャリアのステップを紹介します。
        </p>
        <PillAnchorNav
          items={c.occupations.map((o) => ({
            href: `#${o.id}`,
            label: o.title,
          }))}
        />
        ...
        <div className="mt-2 md:mt-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm md:text-base text-neutral-600">
              ご興味のある方は、募集職種を明記のうえエントリーフォームよりご応募ください。
            </p>
          </div>
          <a
            href={c.ctaHref}
            className="inline-flex items-center justify-center rounded-xl border border-neutral-200 bg-black px-5 py-3 text-white font-semibold shadow-sm hover:opacity-90"
          >
            {c.ctaText}
          </a>
        </div>
